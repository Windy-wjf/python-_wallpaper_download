...
#根据要求下载壁纸,输入分类及要下载的图片页数
num_re = re.compile(r'<div class="pagination">.*?class="selected">1</span><a(?P<page>.*?) href="(?P<pagelink>.*?)">(?P<page_max>.*?)</a>',re.S)
print("您可以选择以下这些分类：aero,animals,architecture,girls,cute,nature,space,travel")
